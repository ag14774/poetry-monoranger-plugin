...
class Monoranger(ApplicationPlugin):
    ...
    def activate(self, application: Application):
        ...
        plugin_config = _merge_dicts(_default_plugin_config(), local_poetry_proj_config)["tool"]["poetry-monoranger-plugin"]
        if not plugin_config["enabled"]:
            return
        
        assert application.event_dispatcher is not None
        application.event_dispatcher.add_listener(cleo.events.console_events.COMMAND, self.console_command_event_listener)
        application.event_dispatcher.add_listener(cleo.events.console_events.TERMINATE, self.post_console_command_event_listener)
